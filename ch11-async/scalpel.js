 ...
 * nest on which it runs. You can use the anyStorage function defined earlier to
 * access storage in arbitrary nests. The scalpel has been going around long enough
...
function locateScalpel2(nest) {
    function search(thisNest) {
        return anyStorage(nest, thisNest, "scalpel").then(nextNest => {
          if (nextNest == thisNest) return thisNest;
          else return search(nextNest);
        });
    }
